...
public class Pintar {   
   ...
   public static void pintarCirculos(Graphics g,int x,int y,String n){
  ...
  public static void pintarCaminho(Graphics g, int x1,int y1,int x2,int y2, Color color){
   ...
   public static void clickSobreEsquinas(Graphics g,int x,int y,String n,Color co){
        //g.drawOval(x, y-10, 20, 20);
       ((Graphics2D)g).setColor(co);
        ((Graphics2D)g).setStroke(new BasicStroke(4));//Espessura do circulo       
        ((Graphics2D)g).fillOval(x, y, 15, 15);        
        ((Graphics2D)g).setColor(Color.BLACK);
        ((Graphics2D)g).drawOval(x, y, 15, 15);
